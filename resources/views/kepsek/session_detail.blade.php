...
<style>
    .modal-backdrop {
        ...
        z-index: 999; overflow-y: auto; padding: 2rem 0;
        align-items: flex-start; justify-content: center; animation: fadeIn 0.3s ease-out;
    }
    .modal-content {
        background: var(--card-bg); border: 1px solid var(--card-border);
        border-radius: var(--radius-lg); width: 90%; max-width: 450px; padding: 2rem;
        box-shadow: var(--card-shadow); text-align: center; margin: auto;
        animation: slideUp 0.4s cubic-bezier(0.16,1,0.3,1);
    }
    .thumbnail-zoom {
        cursor: pointer; transition: var(--transition-smooth);
        border: 1px solid var(--input-border); border-radius: var(--radius-sm); background: white;
    }
    .thumbnail-zoom:hover { transform: scale(1.1); box-shadow: 0 4px 10px rgba(0,0,0,0.15); }
</style>
        ...
        <div class="admin-content-area">
            ...
            <div class="page-header">
                <div>
                    ...
                    <p class="page-subtitle">Rincian kehadiran peserta: <strong>{{ $session->title }}</strong></p>
                </div>
            </div>
                    ...
                    <div id="exportDropdownMenu" style="display:none;position:absolute;right:0;top:calc(100% + 6px);background:var(--card-bg);border:1.5px solid var(--card-border);border-radius:var(--radius-md);box-shadow:var(--card-shadow);min-width:220px;z-index:100;overflow:hidden;">
                        ...
                        <a href="{{ route('kepsek.sessions.export.excel', array_merge(['id' => $session->id], $exportParams)) }}"
                           ...
                           onmouseover="this.style.background='rgba(99,102,241,0.07)'" onmouseout="this.style.background='transparent'">
                            ...
                            <div><div style="font-weight:600;">Unduh Excel</div><div style="font-size:0.75rem;color:var(--text-muted);">Format spreadsheet .xlsx</div></div>
                        </a>
                    </div>
                </div>
            </div>
                            ...
                            <tr>
                                ...
                                <td style="font-family:monospace;color:var(--text-main);">
                                    ...
                                    @if($a->participant && $a->participant->nip)
                                        <div style="font-size:0.72rem;color:var(--text-muted);">NIP: {{ $a->participant->nip }}</div>
                                    @endif
                                </td>
                                <td style="font-weight:600;color:var(--text-main);">{{ $a->participant->name ?? 'Tidak Terdaftar' }}</td>
                                <td><span class="badge badge-info">{{ $a->participant->role ?? 'N/A' }}</span></td>
                                <td>
                                    <div style="font-size:0.85rem;font-weight:600;">{{ $a->signed_in_at->format('H:i:s') }}</div>
                                    <div style="font-size:0.7rem;color:var(--text-muted);">{{ $a->signed_in_at->format('d M Y') }}</div>
                                </td>
                                <td>
                                    @if ($a->location_name)
                                        <div style="font-weight:600;color:var(--text-main);font-size:0.85rem;">{{ $a->location_name }}</div>
                                        <div style="font-size:0.7rem;color:var(--text-muted);margin-top:0.15rem;">{{ $a->latitude }}, {{ $a->longitude }}</div>
                                    @elseif($a->latitude && $a->longitude)
                                        <div style="font-size:0.8rem;font-weight:600;">{{ $a->latitude }}, {{ $a->longitude }}</div>
                                    @else
                                    ...
                                    @if($a->latitude && $a->longitude)
                                        <div style="margin-top:0.35rem;">
                                            <button type="button" class="btn btn-secondary btn-sm"
                                                    ...
                                                    onclick="showMapModal({{ $a->latitude }}, {{ $a->longitude }}, '{{ addslashes($a->participant->name ?? $a->participant_nik) }}')">
                                ...
                                <td style="text-align:center;">
                                    <img src="{{ $a->signature }}" alt="Ttd" class="thumbnail-zoom"
                                         style="height:35px;width:70px;object-fit:contain;"
                                         onclick="showImageZoom('{{ $a->signature }}', 'Tanda Tangan - {{ addslashes($a->participant->name ?? $a->participant_nik) }}')">
                                </td>
                                <td style="text-align:center;">
                                    @if($a->photo)
                                        <img src="{{ $a->photo }}" alt="Selfie" class="thumbnail-zoom"
                                             style="height:35px;width:35px;border-radius:50%;object-fit:cover;"
                                             onclick="showImageZoom('{{ $a->photo }}', 'Foto Selfie - {{ addslashes($a->participant->name ?? $a->participant_nik) }}')">
                                    @else
                        ...
                        <table class="table-custom" style="margin-bottom:0;">
                            <thead style="position:sticky;top:0;z-index:2;">
                                <tr>
                                    ...
                                    <th style="background:#fee2e2;color:#991b1b;border-bottom:1.5px solid rgba(239,68,68,0.2);">Jabatan</th>
                                </tr>
                            </thead>
                            <tbody>
                                ...
                                <tr style="background:rgba(254,242,242,0.25);">
                                    ...
                                    <td style="font-weight:600;color:#991b1b;">{{ $ab->name }}</td>
                                    <td><span class="badge" style="background:#fecaca;color:#991b1b;font-weight:600;font-size:0.75rem;border:1px solid rgba(239,68,68,0.2);">{{ $ab->role }}</span></td>
                                    <td style="color:#991b1b;font-size:0.82rem;font-weight:500;">{{ $ab->jabatan ?? '—' }}</td>
                                </tr>
                                @endforeach
                            </tbody>
                        </table>
